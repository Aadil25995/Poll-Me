...
def add_poll(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Poll and Choices added!',
...
def edit_poll(request, poll_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Poll Edited Sucessfully!',
                             extra_tags='alert alert-success alert-dismissible fade show')
            return redirect('polls:list')

    else:
        form = EditPollForm(instance = poll)
    return render(request,'polls/edit_poll.html', {'poll':poll, 'form':form})
...
def add_choice(request, poll_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_choice.save()
            messages.success(request, 'Choice Added Sucessfully!',
                             extra_tags='alert alert-success alert-dismissible fade show')
            return redirect('polls:list')
    else:
        form = ChoiceForm()
...
def edit_choice(request, choice_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Choice Edited Sucessfully!',
                             extra_tags='alert alert-success alert-dismissible fade show')
            return redirect('polls:list')
    else:
...
def polls_vote(request,poll_id):
    ...
    if choice_id:
        ...
        messages.error(request, 'Error! No Choice!{}'.format(user_voted))
        return redirect('polls:detail', poll_id = poll_id)
    return redirect('polls:detail', poll_id = poll_id)
